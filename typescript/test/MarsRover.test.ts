...
describe("MarsRover", () => {
  ...
  test("given the rover is created, it should have starting point of 0, 0, N", () => {
    ...
    expect(JSON.stringify(createDefaultMarsRover())).toEqual(JSON.stringify(MarsRoverFactory.createMarsRoverFrom(position)));
  });
  ...
  test("given the rover rotate right from north, it should point to east", () => {
    expect(JSON.stringify(createDefaultMarsRover().rotate("R"))).toEqual(JSON.stringify(MarsRoverFactory.createMarsRoverFrom(new Position(0, 0, "E"))));
  });

  test("given the rover rotate left from north, it should point to west", () => {
    expect(JSON.stringify(createDefaultMarsRover().rotate("L"))).toEqual(JSON.stringify(MarsRoverFactory.createMarsRoverFrom(new Position(0, 0, "W"))));
  });
